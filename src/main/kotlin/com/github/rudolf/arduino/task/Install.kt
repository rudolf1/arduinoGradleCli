...
open class Install : DefaultTask() {
    ...
    fun action() {
        ...
        arduinoExt.sketches.forEach { sketch ->
            ...
            val outputDir = File(project.buildDir, "${sketch.path}_${sketch.board}".preparePath())
            val cacheDir = File(project.buildDir, "${sketch.path}_${sketch.board}_cache".preparePath())
            outputDir.mkdirs()
